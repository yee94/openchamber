...
| Tab dock | Four roots. `MobileFloatingBottomBar` 68× stadium, inline inset 16. Selected chrome is a soft pill behind the icon only. | iOS: `UITabBarController`. Android/tests: floating 68px capsule, 5px dock glyphs, 30×18 icon-only selected capsule (primary 7% + 10px glow) + `OcSelectedSpring`. | WidgetTester capsule is opaque `surfaceElevated` + official float-shadow, not `UIGlassEffect`. This VM cannot run an iOS Simulator. |
