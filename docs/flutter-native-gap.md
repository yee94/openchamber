...
|---|---|---|---|
| Projects | Open CJK/Latin tracking, airy meta leading, small thin glyphs. Search smaller than `+`. Nested worktrees, 「更多」 | Type: large title 32 / tracking 1.1 / height 1.22. Session rows 15 / 0.45 / 1.38 with 10px vertical padding. Meta 13 / 0.35 / 1.5. Icons: search 28 / `+` 30 / glyph 11; leading circle 20; dock glyph 13 in 22×18 squircle. Colors stay catalog orange/sand. | WidgetTester dock is opaque, not `UITabBar` / `UIGlassEffect`. Session count is fixture-driven (20, not 50). |
| Chat | Pushed (dock hidden). Airy body leading. Small header / file-type / composer glyphs | Same IA. Body height 1.5 / tracking 0.2. File marks 16×12. Composer `+` 13, send ring 22, scroll FAB 24. User bubble is a primary wash on `OcTokens.card`, not a photo recolor. | **WidgetTester ≠ live iOS glass.** Real iOS keeps `UITextView` + `UIGlassEffect`. No official Grok brand asset. File-type marks are painted MD/TS/TSX. |
| Scheduled | Airy title/meta, small status glyphs, small `+` | Title 16 / 0.4 / 1.32. Meta 13 / 0.35 / 1.5 with 6px gap. Status circle 20 / glyph 9. `+` 30 ink. | Create-task `+` is chrome-only. Overflow `...` still calls run-now. |
