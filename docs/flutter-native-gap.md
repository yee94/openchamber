...
|---|---|---|---|
| Projects | Large title, circular search + filled +, **one** project surface with nested worktrees, 「N 个会话」, relative time, path hint, 「更多」 | Landed. Compact attention strip is title + 「更多」. One `GroupedInsetCard` per project; linked branches nest inside (not separate cards). Worktrees with ≥3 sessions start expanded. Path hint is `Code/github`. | WidgetTester dock is a Material capsule, not `UITabBar`. Session count is fixture-driven (20, not the photo's 50). Warm sand + orange `--primary`, not the photo's lavender + iOS blue tint. |
| Chat | Pushed header (circular back, truncated title, muted busy ring, circular overflow), sparkle + 「Grok 4.6」 + Orchestrator robot pill, purple agent-count, user chrome = clock + undo/edit/fork/link/copy **above** the bubble, copy/share/TTS + tok/s · duration · clock, scroll-to-bottom FAB, solid composer pill. Homepage dock hidden. | Landed as a pushed page only. User toolbar is clock + actions (mention is transcript data, not a second nav). File rows use colored file glyphs + `+N -M`. TTS stays — official Read aloud; the README photo also shows a speaker. | **WidgetTester ≠ live iOS glass.** Android/test composer is a solid pill. Real iOS keeps `UITextView` + `UIGlassEffect`. No official Grok brand asset (sparkle glyph stands in). File-type marks are painted glyphs, not the WebView `FileTypeIcon` sprite. |
| Scheduled | Segmented 任务/历史记录, text filter chips, grey circle + green check / faded pause, 「每天 23:30 · 23h 30m 后」, dock 计划 only | Landed. Selected filter has a soft primary fill. Active badges are a muted circle + green check. Paused cards are faded with a grey pause glyph and `—`. Create `+` is a dark ink circle. | Create-task `+` is chrome-only. Overflow `...` still calls run-now. Selected dock uses theme orange, not the photo's iOS blue-grey tint. |
