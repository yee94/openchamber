...
| Scheduled | Same `MobileTabPageHeader` as Projects. Open title↔meta leading. Shared filter capsule. | Shared overlay header. Title 16 / 1.32 / 1.64, title↔meta 16. Filter chips in one muted track. Paused rows greyscale + 0.78. `+` hit 40 / disc 24. | Create-task `+` is chrome-only. Overflow `...` still calls run-now. |
...
| Tab dock | Four roots. `MobileFloatingBottomBar` 68× stadium, inline inset 16. Selected chrome is a soft pill behind the icon only. | iOS: `UITabBarController`. Android/tests: floating 68px capsule, 6px dock glyphs, 28×16 icon-only selected capsule (primary 11%) + `OcSelectedSpring`. | WidgetTester capsule is opaque `surfaceElevated` + official float-shadow, not `UIGlassEffect`. This VM cannot run an iOS Simulator. |
