...
|---|---|---|---|
| Projects | Shared `MobileTabPageHeader`. One `MobileFloatingSurface` per project; linked worktrees are inset labeled groups inside that surface. | Same hierarchy. Project-shell icon 38 / glyph 18. Session 12 / 16 / −0.012em. Worktree label is `size-3.5` branch, not a second floating card. | WidgetTester frost is `BackdropFilter`, not `UIGlassEffect`. Fixture 20 sessions, not 50. Painted `OcGlyph` ≠ Remix sprites. |
