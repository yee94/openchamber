...
|---|---|---|---|
| Projects | Cool grouped gray `#F2F2F7`, white ~22 radius cards, SF-like large title, white search circle + UIKit-blue `+`, gray leading circles, compact nested sessions, 「更多」, selected dock = blue glyph in a light-blue squircle | `HeroSurface` / `OcIosHero` photograph overlay on this screen. Catalog `OcTokens` stay on the ancestor. Search 32 / `+` 36. Worktrees nest in one project surface; pinned/in-progress stay expanded. | WidgetTester dock is an opaque capsule, not `UITabBar` / `UIGlassEffect`. Session count is fixture-driven (20, not the photo's 50). No live iOS glass. |
| Chat | Pushed (dock hidden). Circular back + truncated title + spinner + `…` only. Cool user bubble, actions **under** the bubble, sparkle + Grok 4.6 + muted Orchestrator pill, small purple agent chip, file-change card with MD/TS/TSX marks, footer metrics, 28px scroll circle, collapsed composer with `+` / 「点击输入」 / send **ring** | Landed as a pushed page. Header has no subtitle. User bubble is `#E8EEF6`, not orange. Composer send is a thin ring (busy = 8px inner square). | **WidgetTester ≠ live iOS glass.** Android/test composer matches spacing/radius/glyph weight only. Real iOS keeps `UITextView` + `UIGlassEffect`. No official Grok brand asset (sparkle stands in). File-type marks are painted MD/TS/TSX, not the WebView `FileTypeIcon` sprite. |
| Scheduled | Slim gray segmented track + white selected pill, text-only filters, small navy `+`, white cards with hollow green check / light-blue pause, paused rows faded, dock 计划 selected in UIKit blue | `HeroSurface` overlay. Segmented control is gray track + white pill (not orange fill). Filters are weight-only. `+` is 28 navy. | Create-task `+` is chrome-only. Overflow `...` still calls run-now. WidgetTester dock is opaque geometry, not glass. |
...
README hero blue on `+` / selected tab is iOS UIKit tint in the photograph (`OcIosHero.systemBlue` `#007AFF`), not `--primary`. Official `--primary` stays orange / golden sand on Settings, Connect, Assistant, and Appearance. If catalog tokens and the README photos conflict on Projects / Chat / Scheduled, **photos win** via `HeroSurface`. Desktop orange must not paint those three screens.
