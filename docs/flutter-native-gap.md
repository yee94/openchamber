...
|---|---|---|---|
| Projects | Shared `MobileTabPageHeader` (sticky/overlay, fixed height, collapse fade). Separate `MobileFloatingSurface` cards: one per project (header + main sessions) and one per linked worktree. No announcement/Live Activity banner. | Overlay scaffold + 6px rest peek (no banner). Search 40px light disc + 14px glyph / stroke 0.50. Session 12 / 1.33 / pad 5 / visual 40 (project-shell; root token stays 46). Title↔meta 3. Float-shadow official geometry 0.04/0.047/0.059 offset 10. Catalog orange/sand. | WidgetTester header fade is a solid color-mix, not UIKit blur. Dock/composer stay opaque. Fixture 20 sessions, not 50. |
| Chat | Pushed (dock hidden). Slim header. Open body leading. Ring send. | Same IA. Plain back chevron. Title 15 / w500. Body 1.45. File rows pad 4. Footer glyphs 9. Composer radius 24, send ring 28 / stroke 0.55. Diff card + composer share the shared float-shadow. | **WidgetTester ≠ live iOS glass.** Real iOS keeps `UITextView` + `UIGlassEffect`. WidgetTester stop-square is expected. |
| Scheduled | Same `MobileTabPageHeader` as Projects. Open title↔meta leading. Shared filter capsule. | Shared overlay header. Project-shell title 14 / 1.29, meta 12 / 1.25, title↔meta 3, pad 10. Status disc 38 / glyph 10. Filter chips in one muted track. Paused greyscale + 0.78. `+` is primary 40. Same shared float-shadow. | Create-task `+` is chrome-only. Overflow `...` still calls run-now. |
...
| Tab dock | Four roots. `MobileFloatingBottomBar` 68× stadium, inline inset 16. Selected chrome is a soft pill behind the icon only. | iOS: `UITabBarController`. Android/tests: floating 68px capsule, 11px dock glyphs (stroke 0.38), 32×22 icon-only selected capsule (primary 9%) + `OcSelectedSpring`. | WidgetTester capsule is opaque `surfaceElevated` + shared float-shadow, not `UIGlassEffect`. This VM cannot run an iOS Simulator. |
