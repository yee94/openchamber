...
|---|---|---|---|
| Projects | Open CJK/Latin tracking on titles + session rows. Small thin glyphs. Search smaller than `+`. Nested worktrees. | Type: large title 32 / 1.6 / 1.28. Session rows 15 / 0.9 / 1.58 with 13px pad. Meta 13 / 0.65 / 1.75. Icons: search 24 / `+` 26 / glyph 8; leading 15; dock 10 in 16×16 squircle. Softer wider card/dock lift. Catalog orange/sand. | WidgetTester dock is opaque, not `UITabBar` / `UIGlassEffect`. Session count is fixture-driven (20, not 50). |
| Chat | Pushed (dock hidden). Slim header. Open body leading. Ring send. | Same IA. Header title 15 / w500 / tracking 0.4. Body height 1.65 / tracking 0.5. File marks 11×8. Composer `+` 10, send ring 17 / stop 4.5, scroll FAB 18. User bubble is a primary wash on `OcTokens.card`, not a photo recolor. | **WidgetTester ≠ live iOS glass.** Real iOS keeps `UITextView` + `UIGlassEffect`. No official Grok brand asset. File-type marks are painted MD/TS/TSX. |
| Scheduled | Open title↔meta leading. Small status glyphs. Shared filter capsule. `+` is catalog primary, not ink black. | Title 16 / 0.8 / 1.48. Meta 13 / 0.65 / 1.75 with 9px gap. Status circle 15 / glyph 6. `+` 26 filled primary. Filter chips sit in one muted track. Paused rows are greyscale + 0.78 fade. | Create-task `+` is chrome-only. Overflow `...` still calls run-now. |
