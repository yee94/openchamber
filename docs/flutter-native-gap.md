...
| Scheduled | Same `MobileTabPageHeader` as Projects. Open title↔meta leading. Shared filter capsule. | Shared overlay header. Title 16 / 1.0 / 1.42, title↔meta 10. Filter chips in one muted track. Paused rows greyscale + 0.78. `+` is catalog primary 40. | Create-task `+` is chrome-only. Overflow `...` still calls run-now. |
