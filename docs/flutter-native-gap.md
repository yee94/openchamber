...
|---|---|---|---|
| Projects | Shared `MobileTabPageHeader` (sticky/overlay, fixed height, collapse fade). Separate `MobileFloatingSurface` cards: one per project (header + main sessions) and one per linked worktree. No announcement/Live Activity banner. | Overlay scaffold unchanged. Search is a 40px light disc + 16px glyph; `+` is primary 40. Session rows 12 / 16 / pad 5 / min 46. Project icon 38 / in-card glyph 11. Float-shadow ~0.024/0.031/0.063 offset 7. Catalog orange/sand. | WidgetTester header fade is a solid color-mix, not UIKit blur. Dock/composer stay opaque. Fixture 20 sessions, not 50. |
| Chat | Pushed (dock hidden). Slim header. Open body leading. Ring send. | Same IA. Plain back chevron. Title 15 / w500. Body 1.56. File rows pad 6. Footer glyphs 10. Composer radius 24, send ring 28. Diff card + composer share the softened float-shadow. | **WidgetTester ≠ live iOS glass.** Real iOS keeps `UITextView` + `UIGlassEffect`. WidgetTester stop-square is expected. |
| Scheduled | Same `MobileTabPageHeader` as Projects. Open title↔meta leading. Shared filter capsule. | Shared overlay header. Title 16 / 1.25, title↔meta 4. Status disc 38 / glyph 13. Filter chips in one muted track. Paused greyscale + 0.78. `+` is primary 40. Same softened float-shadow. | Create-task `+` is chrome-only. Overflow `...` still calls run-now. |
...
| Tab dock | Four roots. `MobileFloatingBottomBar` 68× stadium, inline inset 16. Selected chrome is a soft pill behind the icon only. | iOS: `UITabBarController`. Android/tests: floating 68px capsule, 13px dock glyphs (stroke 0.58), 28×20 icon-only selected capsule + `OcSelectedSpring`. | WidgetTester capsule is opaque `surfaceElevated` + softened float-shadow, not `UIGlassEffect`. This VM cannot run an iOS Simulator. |
