...
| Chat | Pushed header (back, truncated title, busy spinner, overflow), model + Orchestrator, processed duration, agent count, markdown, 「已更改文件」 (5 preview + more), tok/s + duration + clock, collapsed glass pill. Homepage dock hidden. | Landed as a pushed page only (`07-chat.png` is `ChatScreen` as `MaterialApp` home — no Projects chrome, no dock). File preview count matches official `TURN_CHANGES_PREVIEW_VISIBLE_LIMIT` (5). Header spinner follows session busy without expanding settled activity. | WidgetTester is Android: composer is a solid Flutter pill, not `UITextView` + `UIGlassEffect`. Real iOS still uses the UIKit overlay. No Grok brand mark, no message-action icon row, no copy/share/feedback footer — those Capacitor chrome bits are not cloned. |
