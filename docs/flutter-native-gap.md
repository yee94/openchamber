...
| Chat | Pushed (dock hidden). Official `.oc-mobile-detail-navigation`: 56px band, `max(1rem, safe-area)` inset, 40px `mobileGlass` discs, title 0.9375rem / 1.4 centered. Transparent sticky + fade, not a frost banner. | Overlay `PushedNavBar` uses `viewPadding.top` + 56. Back / spinner / `···` are 40px discs. Title 15 / 1.4 / w600 / center. Composer bottom uses `viewPadding.bottom`. | **WidgetTester ≠ live iOS glass.** Real iOS keeps `UITextView` + `UIGlassEffect`. Painted `OcGlyph` ≠ Remix medium sprites 1:1. |
