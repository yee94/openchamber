...
| **Push tap → session** | `deepLinkNavigation.ts` `pushNotificationActionPerformed` | **landed** `sessionDeepLinkFromPushData` + iOS `remoteNotification` / `didReceive` + Android intent extras → `openchamber://session/{id}` | Real APNs/FCM tap on a phone is still residual. |
...
| Push tap → session | landed | `sessionDeepLinkFromPushData` prefers `url`/`deeplink`, else `data.sessionId` → `openchamber://session/{id}`. iOS launch `remoteNotification` + `userNotificationCenter:didReceive`. Android FCM taps use Activity intent extras (`url`/`deeplink`/`sessionId`); `FirebaseMessaging.getInitialMessage` is FlutterFire Dart and is not called. |
