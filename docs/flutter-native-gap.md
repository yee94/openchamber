...
|---|---|---|
| `.github/workflows/flutter-mobile-ci.yml` | **push to `work/flutter-native` only** + `workflow_dispatch` | Parallel `analyze-test` / `android-debug` / `ios-simulator`. Flutter **3.32.8** pinned. No `pull_request`. iOS job is a real `flutter build ios --simulator --no-codesign` and asserts `Runner.app`. **Run #9 (`16a6eb215`):** analyze+Android **success**; iOS failed on damaged `project.pbxproj` (`};` missing). **Run #10 (`4f0449f57`):** analyze+tests **success**, Android APK **success**; iOS compiled past the plist parse then failed Swift `ShareViewController.swift:94` (`UIApplication.open` is async — needs `await`). Fixed in the follow-up commit. **Do not claim Actions green until a later run finishes.** |
