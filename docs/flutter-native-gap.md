...
|---|---|---|---|
| Projects | Large title, circular search + filled +, project/worktree cards, 「N 个会话」, relative time, path hint, nested sessions, 「更多」 | Landed. Compact attention strip is title + 「更多」 (not a 置顶 card, not Dynamic Island). Worktrees with ≥3 sessions start expanded. Path hint is `Code/github`. | WidgetTester dock is a Material capsule, not `UITabBar`. |
| Chat | Pushed header (back, truncated title, busy spinner, overflow), 「Grok 4.6」 + separate Orchestrator pill, purple agent-count, copy/share + tok/s · duration · clock, scroll-to-bottom FAB, collapsed glass pill. Homepage dock hidden. | Landed as a pushed page only. Header shows `project · branch`. User turns have mention + clock/undo/edit/fork/link/copy. Assistant footer is copy / share / TTS + bolt/hourglass/clock — no like/dislike. File preview count matches official `TURN_CHANGES_PREVIEW_VISIBLE_LIMIT` (5). | **WidgetTester ≠ live iOS glass.** The Android/test composer is a solid pill (spacing/radius/glyph weight only). Real iOS keeps `UITextView` + `UIGlassEffect`/`UIBlurEffect`. Do not invent a Flutter glass clone. No official Grok brand mark. |
| Scheduled | Segmented 任务/历史记录, text filter chips, green check / soft-blue pause, 「每天 23:30 · 23h 30m 后」, dock 计划 only | Landed. Selected filter has a soft primary fill. Active badges are filled green. Paused cards are faded with a grey pause glyph and `—`. Create `+` is a dark ink circle. | Create-task `+` is chrome-only. Overflow `...` still calls run-now. |
