...
|---|---|---|---|
| Projects | Shared `MobileTabPageHeader` (sticky/overlay, fixed height, collapse fade). Separate `MobileFloatingSurface` cards: one per project (header + main sessions) and one per linked worktree. No announcement/Live Activity banner. | `MobileTabPageScaffold` is a Stack overlay + layoutHeight slot + 10px expand-shift spacer. Search/`+` hit 40, painted disc 24 / glyph 11. Session rows 13 / 0.55 / 1.38, pad 11 (toward official 12/16 in 46px). Soft integrated float-shadow. Catalog orange/sand. | WidgetTester header fade is a solid color-mix, not UIKit blur. Dock/composer stay opaque. Fixture 20 sessions, not 50. |
| Chat | Pushed (dock hidden). Slim header. Open body leading. Ring send. | Same IA. Plain back chevron (no circular housing). Header title 15 / w500. Body height 1.56 / tracking 0.5. File rows pad 5. Footer glyphs 6. Composer radius 26, send ring 17 / 0.50, no hairline border. Diff card + composer share `--oc-mobile-float-shadow`. | **WidgetTester ≠ live iOS glass.** Real iOS keeps `UITextView` + `UIGlassEffect`. No official Grok brand asset. File-type marks are painted MD/TS/TSX. |
| Scheduled | Same `MobileTabPageHeader` as Projects. Open title↔meta leading. Shared filter capsule. | Shared overlay header. Title 16 / 0.35 / 1.28, title↔meta 8. Filter chips in one muted track. Paused rows greyscale + 0.78. `+` hit 40 / disc 24. Same float-shadow as project cards. | Create-task `+` is chrome-only. Overflow `...` still calls run-now. |
...
| Tab dock | Four roots. `MobileFloatingBottomBar` 68× stadium, inline inset 16. Selected chrome is a soft pill behind the icon only. | iOS: `UITabBarController`. Android/tests: floating 68px capsule, 6px dock glyphs, 30×18 icon-only selected capsule (primary 7% + 8px glow) + `OcSelectedSpring`. | WidgetTester capsule is opaque `surfaceElevated` + official float-shadow, not `UIGlassEffect`. This VM cannot run an iOS Simulator. |
