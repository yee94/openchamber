...
| Chat | Pushed (dock hidden). Slim header. Open body leading. Ring send. | Same IA. Plain back chevron. Title 15 / w500. Body 1.56. File rows pad 6. Footer glyphs 9. Composer radius 24, send ring 28. Diff card + composer share the shared float-shadow. | **WidgetTester ≠ live iOS glass.** Real iOS keeps `UITextView` + `UIGlassEffect`. WidgetTester stop-square is expected. |
