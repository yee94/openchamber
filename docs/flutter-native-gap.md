...
|---|---|---|---|
| Projects | Open CJK/Latin tracking on titles + session rows. Small thin glyphs. Search smaller than `+`. Nested worktrees. | Type restored to Yee-open: large title 32 / 1.6 / 1.28. Session rows 15 / 0.75 / 1.52 with 12px vertical padding. Meta 13 / 0.55 / 1.68. Icons stay small: search 26 / `+` 28 / glyph 9; leading 16; dock 11 in 18×18 squircle. Soft card/dock lift. Catalog orange/sand. | WidgetTester dock is opaque, not `UITabBar` / `UIGlassEffect`. Session count is fixture-driven (20, not 50). |
| Chat | Pushed (dock hidden). Slim header. Open body leading. Ring send. | Same IA. Header title 15 / w500 / tracking 0.4. Body height 1.6 / tracking 0.42. File marks 12×9. Composer `+` 11, send ring 18 / stop 5, scroll FAB 20. User bubble is a primary wash on `OcTokens.card`, not a photo recolor. | **WidgetTester ≠ live iOS glass.** Real iOS keeps `UITextView` + `UIGlassEffect`. No official Grok brand asset. File-type marks are painted MD/TS/TSX. |
| Scheduled | Open title↔meta leading. Small status glyphs. Shared filter capsule. `+` is catalog primary, not ink black. | Title 16 / 0.7 / 1.42. Meta 13 / 0.55 / 1.68 with 9px gap. Status circle 16 / glyph 7. `+` 28 filled primary. Filter chips sit in one muted track. Paused rows are greyscale + 0.78 fade. | Create-task `+` is chrome-only. Overflow `...` still calls run-now. |
