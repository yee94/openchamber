...
|---|---|---|---|
| Projects | Shared `MobileTabPageHeader` (sticky/overlay, fixed height, collapse fade). Separate `MobileFloatingSurface` cards: one per project (header + main sessions) and one per linked worktree. No announcement/Live Activity banner. | `MobileTabPageScaffold` is a Stack overlay + layoutHeight slot + 10px expand-shift spacer. Search/`+` hit 40, painted disc 28 / glyph 14. Session rows 15 / 1.05 / 1.52, pad 7. Soft peripheral float-shadow. Catalog orange/sand. | WidgetTester header fade is a solid color-mix, not UIKit blur. Dock/composer stay opaque. Fixture 20 sessions, not 50. |
| Chat | Pushed (dock hidden). Slim header. Open body leading. Ring send. | Same IA. Plain back chevron (no circular housing). Header title 15 / w500. Body height 1.5 / tracking 0.4. File rows open pad 4. Footer glyphs 7. Diff card + composer share `--oc-mobile-float-shadow`. | **WidgetTester ≠ live iOS glass.** Real iOS keeps `UITextView` + `UIGlassEffect`. No official Grok brand asset. File-type marks are painted MD/TS/TSX. |
| Scheduled | Same `MobileTabPageHeader` as Projects. Open title↔meta leading. Shared filter capsule. | Shared overlay header. Title 16 / 1.28 / 1.52, title↔meta 13. Filter chips in one muted track. Paused rows greyscale + 0.78. `+` hit 40 / disc 28. | Create-task `+` is chrome-only. Overflow `...` still calls run-now. |
...
| Tab dock | Four roots. `MobileFloatingBottomBar` 68× stadium, inline inset 16. Selected chrome is a soft pill behind the icon only. | iOS: `UITabBarController`. Android/tests: floating 68px capsule, 6px dock glyphs, 20px icon-only selected wash (primary 18%) + `OcSelectedSpring`. | WidgetTester capsule is opaque `surfaceElevated` + official float-shadow, not `UIGlassEffect`. This VM cannot run an iOS Simulator. |
