...
| Tab dock | Four roots. `MobileFloatingBottomBar` 68× stadium, inline inset 16. Selected chrome is a soft pill behind the icon only. | iOS: `UITabBarController`. Android/tests: floating 68px capsule, 10px dock glyphs (stroke 0.32), 26×18 icon-only selected capsule (primary 10%) + `OcSelectedSpring`. | WidgetTester capsule is opaque `surfaceElevated` + shared float-shadow, not `UIGlassEffect`. This VM cannot run an iOS Simulator. |
