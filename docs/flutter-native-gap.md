...
|---|---|---|---|
| Projects | Open CJK/Latin tracking, airy meta leading, small thin glyphs. Search smaller than `+`. Nested worktrees, 「更多」 | Type: large title 32 / tracking 1.6 / height 1.28. Session rows 15 / 0.7 / 1.5 with 12px vertical padding. Meta 13 / 0.5 / 1.65. Icons: search 26 / `+` 28 / glyph 10; leading circle 18; dock glyph 12 in 20×16 squircle. Cards use `--oc-mobile-float-shadow` (clip no longer eats the drop). Colors stay catalog orange/sand. | WidgetTester dock is opaque, not `UITabBar` / `UIGlassEffect`. Session count is fixture-driven (20, not 50). |
| Chat | Pushed (dock hidden). Airy body leading. Small header / file-type / composer glyphs | Same IA. Body height 1.55 / tracking 0.35. File marks 14×11. Composer `+` 12, send ring 20, scroll FAB 22. User bubble is a primary wash on `OcTokens.card`, not a photo recolor. | **WidgetTester ≠ live iOS glass.** Real iOS keeps `UITextView` + `UIGlassEffect`. No official Grok brand asset. File-type marks are painted MD/TS/TSX. |
| Scheduled | Airy title/meta, small status glyphs, small `+` | Title 16 / 0.65 / 1.4. Meta 13 / 0.5 / 1.65 with 8px gap. Status circle 18 / glyph 8. `+` 28 ink. Filter 「全部」 is a muted pill. Paused rows are greyscale + 0.78 fade. | Create-task `+` is chrome-only. Overflow `...` still calls run-now. |
