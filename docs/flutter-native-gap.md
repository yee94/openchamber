...
|---|---|---|---|
| Projects | Shared `MobileTabPageHeader` (sticky/overlay, fixed height, collapse fade). Separate `MobileFloatingSurface` cards: one per project (header + main sessions) and one per linked worktree. No announcement/Live Activity banner. | Overlay scaffold + 6px rest peek (no banner). Search 40px glass disc + 20px medium glyph. Session 12 / 1.33 / tracking 0.04 / pad 5 / visual 40. Root title 32 / w600 / −0.04em. Soft-contact float. Catalog orange/sand. | WidgetTester frost is `BackdropFilter`, not `UIGlassEffect`. Fixture 20 sessions, not 50. Mid-scroll proof: `02-projects-scrolled.png`. |
| Chat | Pushed (dock hidden). 44px `mobileGlass` chips. Semibold title. Filled stop disc. Frosted composer over the transcript. | Same IA. Back / spinner / `···` sit in 44px glass chips. Title 15 / w600, left + ellipsis. Composer `+` 26 / stroke 2.4. Stop = filled foreground disc + background rounded square. Overlay frosted pill. | **WidgetTester ≠ live iOS glass.** Real iOS keeps `UITextView` + `UIGlassEffect`. Painted `OcGlyph` ≠ Remix medium sprites 1:1. |
| Scheduled | Same `MobileTabPageHeader` as Projects. Equal-width 全部/已启用/已暂停. 44px white status badge + visible ring. | Shared overlay header. Equal-width filter track. Status disc 44 / glyph 16 / stroke 2.2 + white fill + border. Trailing `···` is a 16px filled button. Paused greyscale + 0.78. `+` is primary 40. | Create-task `+` is chrome-only. Overflow `...` still calls run-now. |
...
| Tab dock | Four roots. `MobileFloatingBottomBar` 68× stadium, inline inset 16. Selected chrome covers the full tab slot. Icons `size-[23px]` medium filled (`folder-open` / sparkling / `calendar-schedule` / `settings-3`). | iOS: `UITabBarController`. Android/tests: floating 68px frosted capsule, 23px filled dock glyphs, full-slot `interactiveSelection` wash + `OcSelectedSpring`. | WidgetTester frost is `BackdropFilter` + `--oc-mobile-glass-fill` 0.68, not `UIGlassEffect`. This VM cannot run an iOS Simulator. |
