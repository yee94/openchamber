...
|---|---|---|---|
| Projects | Large title, circular search + filled +, project/worktree cards, 「N 个会话」, relative time, path hint, nested sessions, 「更多」 | Landed. Cards have hairline separators and tinted type glyphs. Path hint comes from the directory (`Code/github` on the review seed). Homepage does not paint a fake Dynamic Island banner. | WidgetTester dock is a Material capsule, not `UITabBar`. |
| Chat | Pushed header (back, truncated title, busy spinner, overflow), 「Grok 4.6」 + separate Orchestrator pill, purple agent-count, copy/share/feedback + tok/s · duration · clock, scroll-to-bottom FAB, collapsed glass pill. Homepage dock hidden. | Landed as a pushed page only. Orchestrator is a bordered pill with a people glyph. Agent count is a purple sparkle chip. Footer is copy / share / thumbs + bolt/hourglass/clock metrics. File preview count matches official `TURN_CHANGES_PREVIEW_VISIBLE_LIMIT` (5). | **WidgetTester ≠ live iOS glass.** The Android/test composer is a solid pill (spacing/radius/glyph weight only). Real iOS keeps `UITextView` + `UIGlassEffect`/`UIBlurEffect`. Do not invent a Flutter glass clone. No official Grok brand mark. |
| Scheduled | Segmented 任务/历史记录, text filter chips, green check / soft-blue pause, 「每天 23:30 · 23h 30m 后」, dock 计划 only | Landed. Selected filter has a soft primary fill. Status badges are filled green / `OcChrome.pauseAccent` with white glyphs. Paused titles dim. | Create-task `+` is chrome-only. Overflow `...` still calls run-now. |
