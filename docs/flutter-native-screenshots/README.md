...
- WidgetTester runs as Android. Header / dock / composer use `BackdropFilter` + official saturate. Search chips use clear `glassChipFill` (alpha 0) plus contact-only shade — **not** a `UIGlassEffect` clone, inset plate, or 8/20 umbra coin. Primary `+` is contact-only (no control umbra). These PNGs cannot prove live iOS glass or 精致. Real iOS still keeps live glass on UIKit overlays. Mid-scroll proof: `02-projects-scrolled.png`. This Linux VM cannot run an iOS Simulator. Goldens reserve `viewPadding.top = 47` for the status area; they do **not** paint a fake UIKit status bar.
