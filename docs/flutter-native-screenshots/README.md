...
- WidgetTester runs as Android. Header / dock / composer use `BackdropFilter` + official saturate. Search chips use clear `glassChipFill`, **no elevation plate**, and **no chip blur** (page-bleed). Cards use official `--oc-mobile-float-shadow` (2 / 12 / 10-24/-6). Dock selected is mix-only `interactive-selection/55` (no nested frost plate). Dock glyphs are official filled-medium 23px sprites (folder / sparkles / calendar-grid / holed gear). Primary `+` is contact-only. These PNGs cannot prove live iOS glass or 精致. Real iOS still keeps live glass on UIKit overlays. Mid-scroll proof: `02-projects-scrolled.png`. This Linux VM cannot run an iOS Simulator. Goldens reserve `viewPadding.top = 47` for the status area; they do **not** paint a fake UIKit status bar.
