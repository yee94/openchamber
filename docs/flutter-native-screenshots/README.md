...
- WidgetTester runs as Android. Header / dock use `BackdropFilter` + official saturate. Search / `+` use `OcGlassChip` frost plates (`glassChipFill` 0.34, σ14, glass-shadow near-pair — no 8/20 umbra). Project shells keep official `--oc-mobile-float-shadow` (2 / 12 / 10-24/-6). Scheduled tasks use `MobileFloatingSurface(tight: true)` plus 38 project-shell status discs. Dock selected is mix-only `interactive-selection/55` through-mix (no RGB@0.55, no nested frost). Session rows stay in the official ~40 class with a tiny CJK half-lead (0.5 → 42). Folder / sparkles stay official 23px at a lighter visual stroke; calendar/gear paint slimmer filled-medium. Composer is official `--surface-subtle` with no elevation / no frost; idle send is in-pill `send-plane` (no filled disc). These PNGs cannot prove live iOS glass or 精致. Real iOS still keeps live glass on UIKit overlays. Mid-scroll proof: `02-projects-scrolled.png`. This Linux VM cannot run an iOS Simulator. Goldens reserve `viewPadding.top = 47` for the status area; they do **not** paint a fake UIKit status bar.
