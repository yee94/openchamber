...
- WidgetTester runs as Android. Header / dock use `BackdropFilter` + official saturate. Search uses `OcGlassChip` mobileGlass frost (`glassChipFill` 0.34, σ14, official saturate, 32 plate — no 8/20 umbra, no 0.68/36 coin). Solid `+` is official 40 primary. Project and schedule shells keep official `--oc-mobile-float-shadow` (2 / 12 / 10-24/-6, umbra 0x1A). Nested worktree groups keep the official 1px inset border, painted a hair darker so the shell reads on cream. No Material 8/20 umbra. Schedule status discs are 38 project-shell glass. Dock selected is mix-only `interactive-selection/55` (no hairline pill, no RGB@0.55, no nested frost). Session rows stay in the official ~40 class with a 1.25px CJK half-lead (1.25 → 45). Review-face titles (collapsing page title, project/schedule/dock/chat) paint Regular — w600 faux-bold blobs the DroidSansFallback cut. Project leading / worktree glyphs paint under official stroke so Flutter bloom is not a coin. Folder / sparkles stay official 23px at a lighter visual stroke; calendar/gear paint slimmer filled-medium. Composer is official `--surface-subtle` with no elevation / no frost; idle send is in-pill `send-plane` (no filled disc). These PNGs cannot prove live iOS glass or 精致. Real iOS still keeps live glass on UIKit overlays. Mid-scroll proof: `02-projects-scrolled.png`. This Linux VM cannot run an iOS Simulator. Goldens reserve `viewPadding.top = 47` for the status area; they do **not** paint a fake UIKit status bar.
