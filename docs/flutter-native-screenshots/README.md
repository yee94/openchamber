...
- WidgetTester runs as Android. Header / dock / composer use `BackdropFilter` + official saturate. Search chips use quiet `glassChipFill` (0.10) + contact/inset and **no chip blur** — not a 0.68 coin, cream frost disc, or 8/20 umbra. Cards use official float near-pair (2/12) plus a softened far lift (not a 10/24 hard band). Dock selected wash is the authored `--interactive-selection` token (mix-only). Primary `+` is contact-only. These PNGs cannot prove live iOS glass or 精致. Real iOS still keeps live glass on UIKit overlays. Mid-scroll proof: `02-projects-scrolled.png`. This Linux VM cannot run an iOS Simulator. Goldens reserve `viewPadding.top = 47` for the status area; they do **not** paint a fake UIKit status bar.
