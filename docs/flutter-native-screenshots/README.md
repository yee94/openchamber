...
- WidgetTester runs as Android. Header / dock / composer use `BackdropFilter` + official saturate. Search chips use clear `glassChipFill` (alpha 0), contact-only shade, and **no chip blur / inset sheen** so the page shows through — not a cream frost disc or 8/20 umbra coin. Cards use official `--oc-mobile-float-shadow` (2 / 12 / 10-24/-6) plus float blur 22 / saturate 1.35. Primary `+` is contact-only. Dock folder/sparkles share official medium stroke. These PNGs cannot prove live iOS glass or 精致. Real iOS still keeps live glass on UIKit overlays. Mid-scroll proof: `02-projects-scrolled.png`. This Linux VM cannot run an iOS Simulator. Goldens reserve `viewPadding.top = 47` for the status area; they do **not** paint a fake UIKit status bar.
