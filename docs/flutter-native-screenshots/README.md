...
- WidgetTester runs as Android. It **cannot** paint `UIGlassEffect` or a UIKit header material. The shared `MobileTabPageHeader` fade is official `--oc-mobile-header-fade` (surface-background 85%). Real iOS keeps translucency on the existing `UITabBar` overlay — Flutter does not paint a fake-glass header rectangle. This Linux VM cannot run an iOS Simulator.
