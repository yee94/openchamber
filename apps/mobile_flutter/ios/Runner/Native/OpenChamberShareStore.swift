...
enum OpenChamberShareStore {
    ...
    static func target(serverInstanceID: String? = nil, assistantID: String? = nil) -> [String: Any]? {
        // Flutter: never silently substitute a default assistant.
        guard let serverInstanceID, let assistantID, !serverInstanceID.isEmpty, !assistantID.isEmpty else { return nil }
        return catalog().first { entry in
            (entry["enabled"] as? Bool) == true
                && entry["serverInstanceID"] as? String == serverInstanceID
                && entry["assistantID"] as? String == assistantID
        }
    }

    struct ShareConversation {
        let id: String
        let title: String
    ...
    static func loadConversations() -> [ShareConversation] {
        catalog().compactMap { entry in
            guard (entry["enabled"] as? Bool) == true,
                  let server = entry["serverInstanceID"] as? String, !server.isEmpty,
                  let assistant = entry["assistantID"] as? String, !assistant.isEmpty,
                  let id = conversationIdentifier(for: entry) else { return nil }
            return ShareConversation(
                id: id,
                title: (entry["name"] as? String).flatMap { $0.isEmpty ? nil : $0 } ?? assistant,
    ...
    static func writeInbox(
        ...
        guard let target = target(conversationIdentifier: conversationID) else { return }
        let text = (texts + urls).map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }.filter { !$0.isEmpty }.joined(separator: "\n")
        _ = try? write(target: target, text: text.isEmpty ? nil : text, items: [])
    }
...
enum ShareError: LocalizedError { case appGroupUnavailable, missingTarget, tooManyImages, imageTooLarge, totalTooLarge, unsupportedAttachment, imageUnavailable, unrecognizedImage, emptyShare, invalidOperation
    var errorDescription: String? { switch self { case .appGroupUnavailable: return "OpenChamber shared storage is unavailable."; case .missingTarget: return "Choose an exact instance and assistant before sharing."; case .tooManyImages: return "A share supports up to 10 images."; case .imageTooLarge: return "An image exceeds 8 MB."; case .totalTooLarge: return "Shared images exceed 16 MB."; case .unsupportedAttachment: return "Only image attachments are supported."; case .imageUnavailable: return "The shared image could not be loaded. Try sharing it again."; case .unrecognizedImage: return "The shared image format could not be identified safely."; case .emptyShare: return "Add text or at least one image before sharing."; case .invalidOperation: return "A valid operation ID is required." } }
}
