...
3. Chat is pushed from the live session index (pinned/in-progress `项目 · 分支` subtitles). Send/Stop hit `prompt_async` / `abort`. Plus-menu / New Session open a draft (`sessionId == ''`) until first send `POST /api/session`. Live events are `GET /api/global/event` SSE.
...
- Smoke (push to `work/flutter-native` only, plus `workflow_dispatch`): `.github/workflows/flutter-mobile-ci.yml`
  ...
  - Android job uploads `openchamber-flutter-android-debug-apk-<shortsha>` (14-day retention). The file inside is `openchamber-v2-debug-<shortsha>.apk` (`com.yee94.openchamber.debug`). Latest sideload prerelease: [`flutter-v2-debug-852e101`](https://github.com/yee94/openchambery/releases/tag/flutter-v2-debug-852e101) (official launcher icon; isolated from `/releases/latest`).
