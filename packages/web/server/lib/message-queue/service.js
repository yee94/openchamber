...
export const createMessageQueueService = ({ dbPath, getRuntimeConfig = () => null, clock = () => Date.now(), isServerPathAllowed = () => false } = {}) => {
  ...
  const completedIdentity = ({ queueItemID, operationID, messageID }) => {
    const matches = db.prepare('SELECT * FROM queue_completion WHERE queue_item_id = ? OR operation_id = ? OR message_id = ?').all(queueItemID ?? null, operationID ?? null, messageID ?? null);
  ...
  const assertScopeDispatchIdle = (scopeID) => { if (db.prepare(`SELECT 1 FROM queue_item item JOIN queue_attempt attempt ON attempt.queue_item_id=item.queue_item_id
    WHERE item.scope_id=? AND (item.status IN ('sending','reconciling') OR (attempt.lease_token IS NOT NULL AND attempt.lease_expires_at>?))`).get(scopeID, now())) fail('scope_locked'); };
  ...
  const normalizeInterruptedSending = () => {
    ...
    try {
      ...
      db.prepare(`UPDATE queue_item SET status='reconciling',reconciliation_due_at=?,row_version=row_version+1,updated_at=? WHERE queue_item_id IN (${placeholders})`).run(now(), now(), ...ids);
  ...
  const manualSend = (input) => transaction(input?.requestID, input, 'queue:manual-send', (revision, key) => {
    ...
    const row = itemRow(input.queueItemID); if (!row) fail('not_found'); const scope = scopeRow(row.scope_id); if (scope.runtime_key !== key) fail('not_found'); assertScopeRevision(scope, input.expectedRevision); assertRowVersion(row, input.expectedRowVersion); if (!['queued', 'retrying', 'failed', 'unresolved'].includes(row.status)) fail('scope_locked'); assertUnreserved(row.queue_item_id); assertScopeDispatchIdle(scope.scope_id);
    db.prepare('UPDATE queue_item SET position=position+1 WHERE scope_id=? AND position<?').run(scope.scope_id, row.position);
    db.prepare("UPDATE queue_item SET position=0,status='queued',due_at=?,reconciliation_due_at=NULL,dispatch_generation=NULL,last_error_code=NULL,row_version=row_version+1,updated_at=? WHERE queue_item_id=?").run(now(), now(), row.queue_item_id);
  ...
  const reorder = (input) => transaction(input?.requestID, input, 'queue', (revision, key) => {
    ...
    if (!Array.isArray(ids) || new Set(ids).size !== ids.length || ids.length !== current.length || ids.some((id) => !current.includes(id))) fail('validation_error');
  ...
  const claimNext = ({ owner, leaseMs = 30_000, runtimeKey: override } = {}) => {
    ...
    return workerMutation(key, () => {
      ...
      const row = db.prepare(`SELECT item.*, scope.runtime_key FROM queue_item AS item JOIN queue_scope AS scope ON scope.scope_id=item.scope_id
        ...
        ORDER BY item.due_at,item.created_at,item.queue_item_id LIMIT 1`).get(key, now(), now(), now());
      if (!row) return { value: null, scopeIDs: [] };
      const token = crypto.randomUUID(); const expires = now() + leaseMs;
      const updated = db.prepare(`UPDATE queue_item SET status='sending',dispatch_generation=?,row_version=row_version+1,updated_at=? WHERE queue_item_id=? AND status IN ('queued','retrying') AND NOT EXISTS (
        ...
        WHERE active_item.scope_id=? AND (active_item.status IN ('sending','reconciling') OR (active_attempt.lease_token IS NOT NULL AND active_attempt.lease_expires_at>?))
      )`).run(authority.generation, now(), row.queue_item_id, row.scope_id, now());
  ...
  const markWorkerState = (state, { queueItemID, leaseToken, fenceGeneration, errorCode, dueAt = now(), runtimeKey: override } = {}) => workerMutation(capturedRuntimeKey(override), () => {
    ...
    db.prepare('UPDATE queue_item SET status=?,reconciliation_due_at=?,last_error_code=?,row_version=row_version+1,updated_at=? WHERE queue_item_id=?').run(state, reconciling ? dueAt : null, errorCode ?? null, now(), queueItemID);
  ...
  const pauseAuthority = ({ expectedGeneration } = {}) => { const key=runtimeKey(); if (!Number.isInteger(expectedGeneration)) fail('validation_error'); let changed=false; db.exec('BEGIN IMMEDIATE'); try { const current=authorityRow(key); if (current.authority !== 'active' || current.generation !== expectedGeneration) fail('generation_conflict'); const rows=db.prepare("SELECT item.queue_item_id,item.scope_id FROM queue_item item JOIN queue_scope scope ON scope.scope_id=item.scope_id WHERE scope.runtime_key=? AND item.status='sending'").all(key); const ids=rows.map((row)=>row.queue_item_id); if (ids.length) { const marks=ids.map(()=>'?').join(','); db.prepare(`UPDATE queue_item SET status='reconciling',reconciliation_due_at=?,row_version=row_version+1,updated_at=? WHERE queue_item_id IN (${marks})`).run(now(),now(),...ids); db.prepare(`UPDATE queue_attempt SET reconciliation_state='reconciling',reconciliation_started_at=COALESCE(reconciliation_started_at,?),reconciliation_deadline_at=COALESCE(reconciliation_deadline_at,?),reconciliation_next_check_at=?,lease_owner=NULL,lease_token=NULL,lease_expires_at=NULL WHERE queue_item_id IN (${marks})`).run(now(),now()+30_000,now(),...ids); } const generation=current.generation+1; db.prepare("UPDATE queue_runtime SET authority='paused',generation=?,updated_at=? WHERE runtime_key=?").run(generation,now(),key); const revision=globalRevision()+1; touchScopes(rows.map((row)=>row.scope_id),revision); setGlobalRevision(revision); db.exec('COMMIT'); changed=true; return { authority:'paused',generation,revision }; } catch(error){db.exec('ROLLBACK');throw error;} finally {if(changed)publish();} };
