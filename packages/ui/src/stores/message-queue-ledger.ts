...
export const QUEUE_LEDGER_LIMITS = { scopes: 128, totalItems: 2048, attachmentsPerItem: 64, issuesPerItem: 64, string: 8192, content: 131072, dataURL: 25 * 1024 * 1024, bytes: 25 * 1024 * 1024 } as const
...
const parseItem = (v: unknown, normalizeSending: boolean): QueueItemDTO | undefined => {
  const keys = ["version", "queueItemID", "operationID", "messageID", "owner", "content", "attachments", "attachmentIssues", "createdAt", "sendConfig", "status", "attemptCount", "nextAttemptAt", "reconciliationStartedAt", "reconciliationDeadlineAt", "reconciliationChecks", "reconciliationNextCheckAt", "failureKind"]
  ...
  const owner = scope(v.owner), attachments = v.attachments.map(parseQueueAttachmentRefDTO), attachmentIssues = v.attachmentIssues.map(parseIssue)
  if (!owner || !attachments.every(Boolean) || !attachmentIssues.every(Boolean) || new Set((attachments as QueueAttachmentRefDTO[]).map((x) => x.occurrenceRefID)).size !== attachments.length || new Set((attachmentIssues as QueueAttachmentIssueDTO[]).map((x) => x.occurrenceRefID)).size !== attachmentIssues.length || (attachmentIssues as QueueAttachmentIssueDTO[]).some((x) => (attachments as QueueAttachmentRefDTO[]).some((attachment) => attachment.occurrenceRefID === x.occurrenceRefID)) || (v.sendConfig !== undefined && (!plain(v.sendConfig) || !only(v.sendConfig, ["providerID", "modelID", "agent", "variant"]) || !str(v.sendConfig.providerID) || !str(v.sendConfig.modelID) || [v.sendConfig.agent, v.sendConfig.variant].some((x) => x !== undefined && !str(x))))) return undefined
