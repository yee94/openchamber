...
- Markdown file references classify file content before opening. Binary files use the desktop system handler, while image paths continue through the in-app preview path. HTML/HTM paths open the Files/mobile webview preview (`viewerMode: 'preview'`), not the source editor or VS Code `editor.openFile`.
- Mobile surfaces annotate previewable text, image, and HTML paths. Other binary paths remain plain text. HTML preview uses a sandboxed iframe on `/api/fs/serve`.
