...
- `edit` / `multiedit` / `write` stay in `ToolPart` for title + path + diff-stats chrome and use **non-expandable file navigation**. Web/Desktop and dedicated mobile `edit`, `multiedit`, and `apply_patch` clicks open the selected tool's single-file patch and scroll to its first changed line. Web/Desktop renderable-patch fallback opens the target file from the owning turn. Dedicated mobile uses the closable Motion sheet on phones and the right Changes panel on iPad; standard file Changes handles edit fallback, while apply-patch fallback presents every file from the owning turn. VS Code opens its native patch diff. Patch records lacking a parseable file open the owning turn's complete diff. No chevron / expanded diff body.
