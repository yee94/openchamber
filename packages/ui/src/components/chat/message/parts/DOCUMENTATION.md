...
- `edit` / `multiedit` / `write` stay in `ToolPart` for title + path + diff-stats chrome, but are **non-expandable file navigation**: click opens the current turn's file diff and scrolls to the first changed line. VS Code opens its native patch diff. Desktop `apply_patch` opens its owning turn's complete diff with the parsed target file and first changed line selected; patch records without a parseable file open the owning turn's complete diff. Dedicated mobile presents every file from that turn in a closable Motion sheet with `72dvh` / `98dvh` drag snaps and downward-threshold dismissal. No chevron / expanded diff body.
