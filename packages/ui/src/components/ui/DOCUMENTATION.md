...
Phone headers use the shared `Button` `mobileGlass` variant with the `mobileIcon` size for neutral floating circular actions such as back, search, and trailing navigation. These controls must consume that pair instead of rebuilding their background, blur, shadow, focus ring, or 40px geometry locally. Primary create actions keep the same circular geometry but may use the semantic primary fill and matching primary shadow so their action hierarchy remains visible.
