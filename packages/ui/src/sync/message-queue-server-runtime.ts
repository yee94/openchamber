...
export const createMessageQueueServerRuntime = (dependencies: Partial<Dependencies> = {}): MessageQueueServerSurface => {
    ...
    while (!signal.aborted && controller === observer) {
      ...
      try {
        ...
        if (needsApply) {
          ...
          setDescriptors(latest.scopes, capture); publish({ hydration: 'ready', error: undefined }); failures = 0;
          continue;
        }
  ...
  const reloadScope = async (scopeID: string, capture: MessageQueueServerRuntimeCapture, originalError: unknown): Promise<MessageQueueScope | undefined> => {
    ...
    if (known && known.revision > descriptor.revision) {
      return readMessageQueueScope(deps.client, scopeID, known.revision, capture.transportIdentity);
    }
    const next = new Map(state.scopes);
    for (const id of next.keys()) if (!snapshot.scopes.some((scope) => scope.scopeID === id)) next.delete(id);
    next.set(scopeID, descriptor);
    for (const id of state.scopes.keys()) if (!snapshot.scopes.some((scope) => scope.scopeID === id)) clearMessageQueueScope(deps.client, id, capture.transportIdentity);
    setDescriptors([...next.values()], capture);
    replaceMessageQueueSnapshot(deps.client, snapshot, capture.transportIdentity);
    return current;
  };
