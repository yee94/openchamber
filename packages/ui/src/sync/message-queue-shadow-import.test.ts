...
import { createMessageQueueShadowImporter } from './message-queue-shadow-import';
...
const create = (items: QueueItemDTO[], options: { values?: Map<string, Blob | string>; existing?: Array<{ queueItemID: string; operationID: string; messageID: string }>; admit?: (input: unknown) => Promise<unknown>; refresh?: () => Promise<void>; current?: () => boolean; hydration?: 'ready' | 'recovery-required'; degraded?: boolean } = {}) => {
  const releases: string[] = [], admissions: any[] = [], states: any[] = [];
...
describe('message queue shadow importer', () => {
  ...
  test('preserves root/part order, server paths, and releases after admission failure', async () => {
    ...
    expect(setup.admissions[0]?.item.attachments.map((entry: any) => entry.locator.kind)).toEqual(['server-path', 'server-path', 'server-path']);
  });
  ...
  test('keeps bound imports while reporting unbound pending and rejects vscode', async () => {
    ...
    const vscode = attachment('vscode', '["root","vscode"]'); vscode.source = 'vscode'; (vscode as any).vscodePath = '/v'; (vscode as any).vscodeSource = 'file';
  ...
  test('uses fetch for durable URLs and releases tokens for URL failure', async () => {
    ...
    try {
      ...
      const failed = create([item('queued', [url])]); (globalThis.fetch as any) = async () => { throw new Error('network'); }; expect((await failed.importer.run()).status).toBe('error'); expect(failed.releases).toEqual(['queue-queued']);
