...
export type QueueDispatchPayload = { content: string; composerDocument?: QueueItemDTO["composerDocument"]; composerMentions?: QueueItemDTO["composerMentions"]; sendConfig?: QueueItemDTO["sendConfig"]; attachments: Array<{ attachment: QueueItemDTO["attachments"][number]; value: DraftBlobValue }> }
...
export const createMessageQueueDispatcher = (deps: QueueDispatcherDependencies) => {
  ...
  const dispatchWork = async (id: QueueDispatchIdentity, options: QueueDispatchMode = {}, explicitCapture?: QueueRuntimeCapture): Promise<QueueDispatchResult> => {
    ...
    try {
      await deps.post(item.owner as Extract<QueueItemDTO["owner"], { state: "bound" }>, { content: item.content, ...(item.composerDocument ? { composerDocument: item.composerDocument } : {}), ...(item.composerMentions?.length ? { composerMentions: item.composerMentions } : {}), sendConfig: item.sendConfig, attachments: acquired.values }, { ...options, onSendConfirmed: settleConfirmed })
