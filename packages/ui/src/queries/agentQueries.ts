...
export const agentQueryOptions = (
  ...
  return {
    ...
    queryFn: async ({ signal }: { signal: AbortSignal }): Promise<Agent[]> => {
      ...
      const query = normalizedDirectory ? `?directory=${encodeURIComponent(normalizedDirectory)}` : '';
      return Promise.all(agents.map(async (agent) => {
        const response = await runtimeFetch(`/api/config/agents/${encodeURIComponent(agent.name)}${query}`, {
          headers: {
        ...
        if (!response.ok) throw new Error(`Failed to fetch agent metadata for ${agent.name}`);
        const data = await response.json() as { scope?: unknown; sources?: { md?: { exists?: boolean; scope?: unknown; path?: string | null }; json?: { exists?: boolean; scope?: unknown } } };
        const scope = data.scope
          ?? (data.sources?.md?.exists ? data.sources.md.scope : undefined)
          ?? (data.sources?.json?.exists ? data.sources.json.scope : undefined)
          ?? data.sources?.md?.scope
          ?? data.sources?.json?.scope;
        const group = parseAgentGroup(data.sources?.md?.path);
