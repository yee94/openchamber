...
let runtimeIdentity = 'runtime-a'; let runtimeKey = 'runtime-a'; let runtimeGeneration = 1; let send = () => Promise.resolve(); let calls = 0; let sendOptions: unknown; let confirmations = 0; let records: Array<{ info: { id: string } }> = []; let fetchRecords: (_sessionID: string, _messageID: string, _directory: string, options?: { signal?: AbortSignal; timeoutMs?: number }) => Promise<Array<{ info: { id: string } }> | null> = () => Promise.resolve(records); let failure: 'pre-dispatch' | 'ambiguous-dispatched' | 'definitive-rejection' = 'ambiguous-dispatched';
...
import { dispatchQueuedMessage, getAutoReviewBlockedSessions, getQueuedAutoSendRetryDelayMs, planQueueHead, planQueueScheduler, reconcileQueuedMessage } from './useQueuedMessageAutoSend';
...
describe('queued dispatch and scheduler', () => {
  beforeEach(() => { reset(); runtimeIdentity = 'runtime-a'; runtimeKey = 'runtime-a'; runtimeGeneration = 1; calls = 0; confirmations = 0; records = []; fetchRecords = () => Promise.resolve(records); sendOptions = undefined; send = () => Promise.resolve(); failure = 'ambiguous-dispatched'; });
  ...
  test('dispatch routes a duplicate session ID through its owner directory and admitted message identity', async () => { const first = add(); const owner = { ...scope(), directory: '/other-project' }; const second = useMessageQueueStore.getState().addToQueue(owner, { content: 'queued', sendConfig: { providerID: 'p', modelID: 'm' } }); await dispatchQueuedMessage('session-a', { scope: owner }); expect(useMessageQueueStore.getState().getQueueForScope(scope())).toEqual([first]); expect(useMessageQueueStore.getState().getQueueForScope(owner)).toEqual([]); const sent = sendOptions as { directoryHint?: string; messageID?: string }; expect(sent.directoryHint).toBe(owner.directory); expect(sent.messageID).toBe(second.messageID); });
  ...
  test('pre-dispatch and definitive completion clear the dispatch flight with their durable states', async () => {
    ...
    expect(current.messageID).toBe(identity.messageID);
  ...
  test('a flight completion unlocks the next narrow scheduler pass', () => {
    const item = add() as QueueItem;
    const dispatchInFlight = new Set([JSON.stringify([queueScopeKey(scope()), item.queueItemID, item.operationID, item.messageID])]);
    ...
    dispatchInFlight.delete(JSON.stringify([queueScopeKey(scope()), item.queueItemID, item.operationID, item.messageID]));
