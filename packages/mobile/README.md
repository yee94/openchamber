...
- `OpenChamberComposer` is an iOS-only Capacitor overlay. It is not registered on Android; hosted H5 and the web Composer stay unchanged. Appearance Settings on Capacitor iOS exposes `openchamber.iosNativeUi` (default on). Turning it off makes `canUseNativeIosComposer`, the native tab bar, and Live Activities return false so iOS uses the same WebView path as Android.
