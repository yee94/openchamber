...
- One Live Activity for all live working sessions, not just the currently selected top-level session. JS starts it after work stays busy for 5 seconds. The lock screen and expanded Dynamic Island show `{n} working` plus a row per session (title, checkmark or spinner, elapsed `{n}m`). `Activity.request` uses `pushType: .token` so APNs can update that existing Live Activity (token available since iOS 16.1; this is not 17.2 push-to-start). The push token belongs to that Activity instance, not the app-wide device token; native code forwards `{activityId, sessionId, token}` on `pushToken`, including rotations.
