...
- iOS Share Extension collects composer text, `NSExtensionItem.attributedContentText`, URLs, and plain-text providers into `ShareEnvelope.text`; it accepts up to 10 images. Android `ShareReceiverActivity` accepts text, URLs as text, and up to 10 images. Android first copies incoming content into a one-hour app-private draft, presents a compact native confirmation screen, and atomically promotes that same draft ID into the durable inbox only after explicit confirmation. Cancellation removes only the draft; committed operations retain one stable ID across activity recreation and delivery retries. The share contract limits each base64-decoded image to 8 MiB and each operation to 16 MiB; native stores enforce these limits from copied binary byte counts. Inbox records expire after 24 hours; startup and every bridge read remove expired, malformed, interrupted, and acknowledged writes after expiry.
...
- The iOS Share Extension and Android share receiver present compact native confirmation screens with the resolved Assistant, editable shared text, image previews, localized actions, and sending progress before committing content to the inbox.
